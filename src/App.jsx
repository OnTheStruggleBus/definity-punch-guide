...
function MdfPairMap() {
  ...
  const pairNums = Array.from({length:25},(_,i)=>i+1);

  return (
    <div style={{padding:16,display:"flex",flexDirection:"column",gap:12}}>
      {/* Legend */}
      <div style={{display:"flex",gap:6,flexWrap:"wrap",alignItems:"center"}}>
        <div style={{fontSize:9,color:"#546E7A",letterSpacing:2,marginRight:4}}>LEGEND:</div>
        {Object.entries(SIG_STYLES).map(([k,v])=>(
          <div key={k} style={{display:"flex",alignItems:"center",gap:4}}>
            <div style={{width:20,height:20,borderRadius:3,background:v.bg,
              border:`1px solid ${v.border}`,display:"flex",alignItems:"center",
              justifyContent:"center",fontSize:7,color:v.border,fontFamily:"'Share Tech Mono',monospace",
              fontWeight:"bold"}}>{v.label}</div>
            <div style={{fontSize:9,color:"#546E7A"}}>{v.desc}</div>
          </div>
        ))}
        <div style={{display:"flex",alignItems:"center",gap:4}}>
          <div style={{width:20,height:20,borderRadius:3,background:"#0A0E1A",
            border:"1px solid #1E2A35"}}/>
          <div style={{fontSize:9,color:"#546E7A"}}>Unused</div>
        </div>
      </div>

      <div style={{fontSize:9,color:"#455A64",lineHeight:1.7}}>
        Source: Avaya official cross-connect diagram (widfccf2 KLC 063097). Each column = one Amphenol 25-pair position.
        Hover a cell to see signal details.
      </div>

      {/* Pair number header */}
      <div style={{overflowX:"auto"}}>
        <div style={{minWidth:700}}>
          <div style={{display:"grid",gridTemplateColumns:"140px repeat(25,1fr)",gap:2,marginBottom:4}}>
            <div style={{fontSize:8,color:"#546E7A",display:"flex",alignItems:"center",paddingLeft:4}}>CARD</div>
            {pairNums.map(n=>(
              <div key={n} style={{fontSize:8,color:n===25?"#546E7A":"#455A64",
                textAlign:"center",fontFamily:"'Share Tech Mono',monospace",
                fontWeight:n===25?"normal":"bold"}}>{n}</div>
            ))}
          </div>

          {/* Wire color row */}
          <div style={{display:"grid",gridTemplateColumns:"140px repeat(25,1fr)",gap:2,marginBottom:8}}>
            <div style={{fontSize:7,color:"#37474F",paddingLeft:4,display:"flex",alignItems:"center"}}>25-pair color</div>
            {PAIR_25.map((p,i)=>(
              <div key={i} style={{display:"flex",flexDirection:"column",gap:1,alignItems:"center"}}>
                <div style={{width:"100%",height:6,background:p.tipHex,
                  borderTop:`1px solid ${p.tipStripe||p.tipHex}`}}/>
                <div style={{width:"100%",height:6,background:p.ringHex}}/>
              </div>
            ))}
          </div>
          ...
          {/* Card rows */}
          {CARD_MAPS.map(card=>(
            <div key={card.tn} style={{display:"grid",
              gridTemplateColumns:"140px repeat(25,1fr)",gap:2,marginBottom:3}}>
              {/* Card label */}
              <div style={{display:"flex",flexDirection:"column",justifyContent:"center",paddingLeft:4}}>
                <div style={{fontFamily:"'Barlow Condensed',sans-serif",fontSize:11,fontWeight:700,
                  color:card.color,letterSpacing:0.5}}>{card.tn}</div>
                <div style={{fontSize:8,color:"#455A64",lineHeight:1.2}}>{card.name}</div>
              </div>
              {/* Pair cells */}
              {card.pairs.map((p,i)=>{
                const hKey=`${card.tn}-${i}`;
                const isHov=hovered===hKey;
                const style=p?SIG_STYLES[p.type]:null;
                return (
                  <div key={i}
                    ...
                    style={{
                      ...
                      background:style?style.bg:"#0A0E1A",
                      border:`1px solid ${style?(isHov?"#FFFFFF":style.border):"#1E2A35"}`,
                    ...
                    {style&&(
                      <div style={{fontSize:6,color:style.border,fontFamily:"'Share Tech Mono',monospace",
                        fontWeight:"bold",lineHeight:1,textAlign:"center"}}>
                        {style.label}
                        {p.port>0&&<div style={{fontSize:5,opacity:0.8}}>{p.port}</div>}
                      </div>
                    )}
                    {isHov&&p&&(
                      <div style={{
                        ...
                        background:"#0D1117",border:`1px solid ${style.border}`,
                        borderRadius:4,padding:"4px 6px",
                        fontSize:9,color:"#CFD8DC",whiteSpace:"nowrap",
                        zIndex:100,pointerEvents:"none",
                        fontFamily:"'Share Tech Mono',monospace",
                      }}>
                        {card.tn} P{p.port} · {style.desc}<br/>
                        Amp pair {i+1} · {PAIR_25[i]?.tip}/{PAIR_25[i]?.ring}
                      </div>
                    )}
                  </div>
                );
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(220px,1fr))",gap:6,marginTop:4}}>
        {CARD_MAPS.map(card=>(
          <div key={card.tn} style={{background:"#0A131C",border:"1px solid #1E2A35",
            borderRadius:5,padding:"8px 10px",borderLeft:`3px solid ${card.color}`}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"baseline",marginBottom:3}}>
              <div style={{fontFamily:"'Barlow Condensed',sans-serif",fontSize:13,fontWeight:700,
                color:card.color}}>{card.tn}</div>
              <div style={{fontSize:9,color:"#455A64"}}>{card.ports} ports</div>
            </div>
            <div style={{fontSize:9,color:"#78909C",lineHeight:1.6}}>{card.desc}</div>
          </div>
        ))}
      </div>
    </div>
